...
export async function GET(request) {
  ...
  try {
    ...
    if (existing) {
      return Response.json({ message: `Case for ${tomorrow} already exists` });
    }

    // Generate and store
    const caseData = await generateCase(tomorrow);
    const { error } = await supabase
      .from('daily_cases')
      .insert({ date: tomorrow, ...caseData });

    if (error) throw new Error(error.message);

    return Response.json({ message: `Case for ${tomorrow} generated successfully`, diagnosis: caseData.diagnosis });
  } catch (err) {
